...
const BOOK_MAP = {
  'gen': 'GEN', 'genesis': 'GEN',
  'ex': 'EXO', 'exodus': 'EXO', 'exo': 'EXO',
  'lev': 'LEV', 'leviticus': 'LEV',
  'num': 'NUM', 'numbers': 'NUM',
  'deut': 'DEU', 'deuteronomy': 'DEU', 'deu': 'DEU',
  'josh': 'JOS', 'joshua': 'JOS', 'jos': 'JOS',
  'judg': 'JDG', 'judges': 'JDG', 'jdg': 'JDG',
  'ruth': 'RUT', 'ruth': 'RUT',
  '1 sam': '1SA', '1 samuel': '1SA',
  '2 sam': '2SA', '2 samuel': '2SA',
  '1 ki': '1KI', '1 kings': '1KI', '1 kgs': '1KI',
  '2 ki': '2KI', '2 kings': '2KI', '2 kgs': '2KI',
  '1 chr': '1CH', '1 chronicles': '1CH',
  '2 chr': '2CH', '2 chronicles': '2CH',
  'ezra': 'EZR', 'ezra': 'EZR',
  'neh': 'NEH', 'nehemiah': 'NEH',
  'esth': 'EST', 'esther': 'EST', 'est': 'EST',
  'ps': 'PSA', 'psalms': 'PSA', 'psa': 'PSA',
  'prov': 'PRO', 'proverbs': 'PRO', 'pro': 'PRO',
  'eccl': 'ECC', 'ecclesiastes': 'ECC', 'ecc': 'ECC',
  'song': 'SNG', 'isa': 'ISA', 'isaiah': 'ISA',
  'jer': 'JER', 'jeremiah': 'JER',
  'lam': 'LAM', 'lamentations': 'LAM',
  'ezek': 'EZK', 'ezekiel': 'EZK', 'eze': 'EZK',
  'dan': 'DAN', 'daniel': 'DAN',
  'hos': 'HOS', 'hosea': 'HOS',
  'joel': 'JOL', 'joel': 'JOL',
  'amos': 'AMO', 'amos': 'AMO',
  ...
  'hag': 'HAG', 'haggai': 'HAG',
  'zech': 'ZEC', 'zechariah': 'ZEC', 'zec': 'ZEC',
  'mal': 'MAL', 'malachi': 'MAL',
  ...
  'acts': 'ACT', 'acts': 'ACT',
  'rom': 'ROM', 'romans': 'ROM',
  '1 cor': '1CO', '1 corinthians': '1CO',
  '2 cor': '2CO', '2 corinthians': '2CO',
  'gal': 'GAL', 'galatians': 'GAL',
  'eph': 'EPH', 'ephesians': 'EPH',
  'phil': 'PHP', 'philippians': 'PHP',
  'col': 'COL', 'colossians': 'COL',
  '1 thess': '1TH', '1 thessalonians': '1TH',
  '2 thess': '2TH', '2 thessalonians': '2TH',
  '1 tim': '1TI', '1 timothy': '1TI',
  '2 tim': '2TI', '2 timothy': '2TI',
  'tit': 'TIT', 'titus': 'TIT',
  'phm': 'PHM', 'philemon': 'PHM',
  'heb': 'HEB', 'hebrews': 'HEB',
  'jas': 'JAS', 'james': 'JAS', 'jms': 'JAS',
  '1 pet': '1PE', '1 peter': '1PE', '1 pe': '1PE',
  '2 pet': '2PE', '2 peter': '2PE', '2 pe': '2PE',
  '1 jn': '1JN', '1 john': '1JN',
  '2 jn': '2JN', '2 john': '2JN',
  '3 jn': '3JN', '3 john': '3JN',
  'jude': 'JUD', 'jude': 'JUD',
...
// Fetch from bible.helloao.org (MBBTAG - Tagalog)
const fetchMBBTAG = async (refObj) => {
  try {
    if (!refObj.usfm || !HELLO_AO_BOOKS[refObj.usfm]) return [];
    const bookPath = HELLO_AO_BOOKS[refObj.usfm];
    
    // This API returns the WHOLE chapter
    const url = `https://bible.helloao.org/api/MBBTAG12/${bookPath}/${refObj.chapter}.json`;
    const { data } = await axios.get(url, { timeout: 10000 });
    
    if (!data || !data.chapter || !data.chapter.verses) return [];
...
module.exports = async (req, res) => {
  ...
  if (!refObj) return res.status(400).json({ error: "Invalid reference format" });

  console.log(`Processing Cloud API Request for: ${ref}`);
  ...
  try {
    ...
    if (kjvV.length > 0 || mbbV.length > 0) {
      ...
      return res.status(404).json({ error: "Verses not found in any cloud API." });
    }
  } catch (e) {
